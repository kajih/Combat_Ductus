...
- **LAN play**: `Trunk.toml` sets `[serve] address = "0.0.0.0"` so other machines on the LAN can load the client — Trunk otherwise binds loopback only. Use the singular `address` key: the `addresses` list form is silently ignored by trunk 0.21.14, which then logs a normal "server listening at" banner (LAN IPs and all) while binding nothing. Trunk 0.21.14 also warns that `address` is deprecated in favour of `addresses` — ignore that warning; following it is what breaks the bind. Trust `ss -tln | grep 8080`, not the banner.
- **Trunk's banner comes ~45-50 seconds before the port is actually listening** (measured on the Linux laptop). In that window the banner lists every LAN IP, `ss -tln | grep 8080` shows nothing, and `curl` gets connection refused — which looks exactly like the silent-no-bind failure above, but isn't. Poll until the port appears (`until ss -tln | grep -q :8080; do sleep 2; done`) before concluding anything is wrong; a single check a few seconds after the banner will mislead you.
- **Disable vim-keybinding browser extensions for the dev-server URL.** Vimium binds `d`, `j`, `k` and `h` by default, which silently swallows Move Right, Punch, Kick and Motivational Speech before they ever reach the canvas — `a` is unbound, so Move Left keeps working and the game looks like it has a one-directional movement bug. The server never sees the input at all, so no amount of reading `input.rs` finds it. Fix: Vimium Options → *Excluded URLs and keys* → add `http://*:8080/*` with an empty keys field (an incognito window works as a quick check, since extensions are off there by default).
